...
int main(void) {
    ...
    fossil_io_soap_add_custom_filter("badword");
    fossil_io_soap_add_custom_filter("anotherbadword");

    // Apply the filter to the input text
    // The filter will process the text and remove or mask the specified words
    char *filtered = fossil_io_soap_filter("badword,anotherbadword", text);

    // Output the filtered result
    fossil_io_printf("Filtered: %s\n", filtered);

    // Free the dynamically allocated filtered string to prevent memory leaks
    free(filtered);
