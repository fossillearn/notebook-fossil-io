 ...
 *  - Calls fossil_io_display_menu() to present the menu to the user and capture their selection.
...
int main(void) {
    ...
    int selection = fossil_io_display_menu("Select an option:", choices, 3);
    // Print the selected option to the terminal
    fossil_io_printf("You selected: %s\n", choices[selection]);
