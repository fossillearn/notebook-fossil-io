 ...
 */
int main(void) {
    // Create a parser palette for CLI commands using the C++ wrapper
    fossil_io_parser_palette_t *palette = fossil::io::Parser::create_palette("toolkit", "Example CLI toolkit");

    // Add commands to the palette using the C++ wrapper
    fossil::io::Parser::add_command(palette, "start", "Start the service");
    fossil::io::Parser::add_command(palette, "stop", "Stop the service");
    fossil::io::Parser::add_command(palette, "restart", "Restart the service");

    // Iterate through the commands and print their details
    fossil_io_parser_command_t *cmd = palette->commands;
    while (cmd) {
        fossil::io::Output::printf("Command: %s - %s\n", cmd->name, cmd->description);
        cmd = cmd->next;
    }

    // Free the palette to release resources using the C++ wrapper
