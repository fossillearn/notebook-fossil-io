...
int main(void) {
       ...
       std::string test_str = "SELECT * FROM users WHERE name='admin'; <script>alert(1)</script> ../../etc/passwd";
       ...
       int flags = fossil::io::Input::validate_sanitize_string(test_str, ctx);

       // Print the original and sanitized strings, as well as the flags.
       fossil::io::Output::printf("Original: %s\n", "SELECT * FROM users WHERE name='admin'; <script>alert(1)</script> ../../etc/passwd");
       fossil::io::Output::printf("Sanitized: %s\n", test_str.c_str());
       fossil::io::Output::printf("Flags: 0x%02X\n", flags);

       // Demonstrate alphanumeric validation.
       const char *alnum = "Test123";
       fossil::io::Output::printf("%s is alphanumeric? %s\n", alnum,
              fossil::io::Input::validate_is_alnum(alnum) ? "Yes" : "No");

       // Demonstrate email validation.
       const char *email = "user@example.com";
       fossil::io::Output::printf("%s is email? %s\n", email,
              fossil::io::Input::validate_is_email(email) ? "Yes" : "No");

       // Demonstrate length validation.
       const char *long_str = "Short";
       fossil::io::Output::printf("%s within length 10? %s\n", long_str,
              fossil::io::Input::validate_is_length(long_str, 10) ? "Yes" : "No");
