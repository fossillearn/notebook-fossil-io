 ...
 */
int main(void) {
    cstring safe = fossil_io_cstring_create_safe("A very long string", 32);
    if (!safe) {
        fossil_io_printf("Failed to create safe string!\n");
        return 1;
    }

    fossil_io_printf("Safe string: %s\n", safe);

    fossil_io_cstring_free_safe(&safe);
    return 0;
}
