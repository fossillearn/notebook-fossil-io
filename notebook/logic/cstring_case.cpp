...
int main(void) {
    fossil::io::CString text("Hello World, HELLO Universe!");

    // Case-insensitive search
    if (text.icontains("hello")) {
        int offset = text.index_of("Hello");
        fossil::io::Output::printf("Found at offset: %d\n", offset);
    }

    // Case-insensitive replace (manual logic)
    fossil::io::CString replaced = text.replace("Hello", "Hi").replace("HELLO", "Hi");
    fossil::io::Output::printf("After replace: %s\n", replaced.str());
